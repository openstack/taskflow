...
class RetryTest(utils.EngineTestBase):
    ...
    def test_states_retry_success_linear_flow(self):
        flow = lf.Flow('flow-1', retry.Times(4, 'r1', provides='x')).add(
            utils.ProgressingTask("task1"), utils.ConditionalTask("task2")
        )
        engine = self._make_engine(flow)
    ...
    def test_states_retry_failure_parent_flow_fails(self):
        ...
        with utils.CaptureListener(engine) as capturer:
            engine.run()
        self.assertEqual(
            {'y': 2, 'x1': 2, 'x2': 1}, engine.storage.fetch_all()
        )
        expected = [
            'flow-1.f RUNNING',
            'r1.r RUNNING',
    ...
    def test_restart_reverted_unordered_flows_with_retries(self):
        ...
        subsubflow1 = lf.Flow(
            'subflow1.subsubflow1', retry=utils.RetryFiveTimes()
        )
        subsubflow1.add(
            utils.SuccessAfter3Sec(
                'subflow1.fail1', inject={'start_time': now}
            )
        )
        ...
        subsubflow2 = lf.Flow(
            'subflow1.subsubflow2', retry=utils.RetryFiveTimes()
        )
        ...
        subsubflow1 = lf.Flow(
            'subflow2.subsubflow1', retry=utils.AlwaysRetry()
        )
        ...
        if isinstance(
            engine._task_executor, executor.ParallelGreenThreadTaskExecutor
        ):
            self.skipTest("Skipping this test when using green threads.")

        with utils.CaptureListener(engine) as capturer:
            self.assertRaisesRegex(
                exc.WrappedFailure, '.*RuntimeError: Woot!', engine.run
            )
    ...
    def test_resume_flow_that_should_be_retried(self):
        flow = lf.Flow('flow-1', retry.Times(3, 'r1')).add(
            utils.ProgressingTask('t1'), utils.ProgressingTask('t2')
        )
        engine = self._make_engine(flow)
        engine.compile()
        engine.prepare()
    ...
    def test_retry_tasks_that_has_not_been_reverted(self):
        flow = lf.Flow('flow-1', retry.Times(3, 'r1', provides='x')).add(
            utils.ConditionalTask('c'), utils.ProgressingTask('t1')
        )
        engine = self._make_engine(flow)
        engine.storage.inject({'y': 2})
        with utils.CaptureListener(engine) as capturer:
    ...
    def test_default_times_retry(self):
        flow = lf.Flow('flow-1', retry.Times(3, 'r1')).add(
            utils.ProgressingTask('t1'), utils.FailingTask('t2')
        )
        engine = self._make_engine(flow)
        with utils.CaptureListener(engine) as capturer:
            self.assertRaisesRegex(RuntimeError, '^Woot', engine.run)
    ...
    def test_nested_parameterized_for_each_revert_all(self):
        values = [3, 2, 5]
        retry1 = retry.ParameterizedForEach(
            'r1', provides='x', revert_all=True
        )
        flow = lf.Flow('flow-1').add(
            utils.ProgressingTask('task-1'),
            lf.Flow('flow-2', retry1).add(
...
class RetryParallelExecutionTest(utils.EngineTestBase):
    ...
    def test_when_subflow_fails_revert_running_tasks(self):
        waiting_task = utils.WaitForOneFromTask(
            'task1', 'task2', [st.SUCCESS, st.FAILURE]
        )
        flow = uf.Flow('flow-1', retry.Times(3, 'r', provides='x')).add(
    ...
    def test_when_subflow_fails_revert_success_tasks(self):
        waiting_task = utils.WaitForOneFromTask(
            'task2', 'task1', [st.SUCCESS, st.FAILURE]
        )
        flow = uf.Flow('flow-1', retry.Times(3, 'r', provides='x')).add(
            utils.ProgressingTask('task1'),
            lf.Flow('flow-2').add(
                waiting_task, utils.ConditionalTask('task3')
            ),
        )
        engine = self._make_engine(flow)
        engine.atom_notifier.register('*', waiting_task.callback)
...
class ParallelEngineWithThreadsTest(
    RetryTest, RetryParallelExecutionTest, test.TestCase
):
    _EXECUTOR_WORKERS = 2

    def _make_engine(
        self, flow, defer_reverts=None, flow_detail=None, executor=None
    ):
        if executor is None:
            executor = 'threads'
        return taskflow.engines.load(
