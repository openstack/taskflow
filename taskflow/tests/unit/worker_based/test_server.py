...
class TestServer(test.MockTestCase):
    ...
    def test_parse_request_with_success_result(self):
        ...
        self.assertEqual(
            (
                ...
                self.task.name,
                'revert',
                dict(arguments=self.task_args, result=1),
            ),
            (task_cls, task_name, action, task_args),
        )
    ...
    def test_parse_request_with_failures(self):
        failures = {
            '0': failure.Failure.from_exception(Exception('test1')),
        ...
        self.assertEqual(
            (
                ...
                dict(
                    arguments=self.task_args,
                    failures={
                        i: utils.FailureMatcher(f) for i, f in failures.items()
                    },
                ),
            ),
            (task_cls, task_name, action, task_args),
    ...
    def test_start(self):
        ...
        # check calls
        master_mock_calls = [mock.call.proxy.start()]
        self.master_mock.assert_has_calls(master_mock_calls)
    ...
    def test_wait(self):
        ...
        # check calls
        master_mock_calls = [mock.call.proxy.start(), mock.call.proxy.wait()]
        self.master_mock.assert_has_calls(master_mock_calls)
    ...
    def test_stop(self):
        ...
        # check calls
        master_mock_calls = [mock.call.proxy.stop()]
        self.master_mock.assert_has_calls(master_mock_calls)
