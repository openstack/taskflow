...
class FlowDependenciesTest(test.TestCase):
    ...
    def test_linear_flow_requires_values(self):
        flow = lf.Flow('lf').add(
            utils.TaskOneArg('task1'), utils.TaskMultiArg('task2')
        )
        self.assertEqual({'x', 'y', 'z'}, flow.requires)
        self.assertEqual(set(), flow.provides)
    ...
    def test_linear_flow_multi_provides_and_requires_values(self):
        flow = lf.Flow('lf').add(
            utils.TaskMultiArgMultiReturn(
                'task1', rebind=['a', 'b', 'c'], provides=['x', 'y', 'q']
            ),
    ...
    def test_unordered_flow_multi_provides_and_requires_values(self):
        flow = uf.Flow('uf').add(
            utils.TaskMultiArgMultiReturn(
                'task1', rebind=['a', 'b', 'c'], provides=['d', 'e', 'f']
            ),
            utils.TaskMultiArgMultiReturn('task2', provides=['i', 'j', 'k']),
        )
        ...
        flow = uf.Flow('uf').add(
            lf.Flow('lf').add(
                utils.TaskOneArgOneReturn(
                    'task1', rebind=['a'], provides=['x']
                ),
                utils.TaskOneArgOneReturn('task2', provides=['y']),
            ),
            uf.Flow('uf').add(
                utils.TaskOneArgOneReturn(
                    'task3', rebind=['b'], provides=['z']
                ),
                utils.TaskOneArgOneReturn(
                    'task4', rebind=['c'], provides=['q']
                ),
            ),
        )
        self.assertEqual({'a', 'b', 'c'}, flow.requires)
    ...
    def test_task_requires_and_provides_same_values(self):
        flow = lf.Flow(
            'lf', utils.TaskOneArgOneReturn('rt', requires='x', provides='x')
        )
    ...
    def test_linear_flow_retry_and_task(self):
        ...
        flow.add(
            utils.TaskMultiArgOneReturn(rebind=['a', 'x', 'c'], provides=['z'])
        )
