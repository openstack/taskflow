...
class TestDeciders(test.TestCase):
    def test_translate(self):
        ...
        for val in ['atom', 'ATOM', 'atOM', deciders.Depth.ATOM]:
            self.assertEqual(
                deciders.Depth.ATOM, deciders.Depth.translate(val)
            )
        for val in [
            'neighbors',
            'Neighbors',
            ...
            self.assertEqual(
                deciders.Depth.NEIGHBORS, deciders.Depth.translate(val)
            )
        for val in ['flow', 'FLOW', 'flOW', deciders.Depth.FLOW]:
            self.assertEqual(
                deciders.Depth.FLOW, deciders.Depth.translate(val)
            )
    ...
    def test_pick_widest(self):
        ...
        self.assertEqual(
            deciders.Depth.NEIGHBORS, deciders.pick_widest(choices)
        )
